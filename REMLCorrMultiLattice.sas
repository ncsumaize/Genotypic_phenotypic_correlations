...
%macro varcomp(trait);
proc mixed data = one;
class env rep block geno;
model &trait = ;
random env rep(env) block(rep*env) geno geno*env;
*also check effect of setting environments, reps, and blocks fixed on other variance components;
proc mixed data = one;
class env rep block geno;
model &trait = env rep(env) block(rep*env);
