...
data one; set one; if env = 98;proc print;

*first, estimate variance components for each trait separately to compare to multivariate analysis below;
%macro varcomp(trait);
proc mixed data = one;
class rep block geno;
model &trait = ;
random rep block(rep) geno ;
*also check effect of setting reps and blocks fixed on other variance components;
proc mixed data = one;
class rep block geno;
model &trait = rep block(rep);
random geno;
run;
%mend;
